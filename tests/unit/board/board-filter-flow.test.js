...
it('유효한 표시 개수를 선택 상자와 게시글 API에 반영한다', async () => {
  ...
  expect(outlet.querySelector('[data-page-size-value]').textContent).toBe('20개씩 보기');
...
it('표시 개수 서브메뉴에서 옵션을 선택한다', () => {
  const { outlet, navigate } = mount();
  const trigger = outlet.querySelector('.board-page-size__trigger');
  const menu = outlet.querySelector('.board-page-size__menu');

  trigger.click();
  expect(trigger.getAttribute('aria-expanded')).toBe('true');
  expect(menu.hidden).toBe(false);
  expect(menu.querySelectorAll('.board-page-size__option')).toHaveLength(3);

  menu.querySelector('[data-page-size="30"]').click();
  expect(outlet.querySelector('[name="page-size"]').value).toBe('30');
  expect(trigger.textContent.trim()).toBe('30개씩 보기');
