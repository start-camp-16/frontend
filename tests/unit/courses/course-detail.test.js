...
it('거리를 숨기고 비밀번호 오류 뒤에도 편집 초안을 유지해 수정·삭제한다', async () => {
  ...
  document.querySelector('[name="delete-password"]').value = '1';
  [...document.querySelectorAll('dialog button')].find(button => button.textContent === '삭제').click();
  await vi.waitFor(() => expect(document.querySelector('dialog').textContent).toContain('4~20자'));
  expect(fetch.mock.calls.some(([input, options]) => new URL(input).pathname.includes('/api/courses/') && options.method === 'DELETE')).toBe(false);
