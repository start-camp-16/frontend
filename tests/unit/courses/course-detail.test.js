...
beforeEach(() => {
  ...
  vi.stubGlobal('fetch', vi.fn(async (input, options) => {
    ...
    if (url.pathname === `/api/courses/${publicId}` && options.method === 'PUT') {
      ...
      return json({ ...course, title: '수정된 코스', updated_at: '2026-07-15T01:00:00Z', stops: [course.stops[1], course.stops[0], course.stops[2]] });
    }
...
it('거리를 숨기고 비밀번호 오류 뒤에도 편집 초안을 유지해 수정·삭제한다', async () => {
  ...
  expect([...root.querySelectorAll('[data-course-stop]')].map(item => item.dataset.courseStop)).toEqual(['2', '1', '3']);
  ...
  expect(JSON.parse(putCalls[1][1].body).location_content_ids).toEqual(['2', '1', '3']);
  app.stop();
});
