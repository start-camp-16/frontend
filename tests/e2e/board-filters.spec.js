...
test('지역 모달과 테마 메뉴로 게시판을 필터링한다', async ({ page }) => {
  ...
  await expect(page).toHaveURL('/posts?district=%EB%A7%88%ED%8F%AC%EA%B5%AC&size=10');

  await page.getByRole('button', { name: '맛집', exact: true }).click();
  await expect(page).toHaveURL('/posts?district=%EB%A7%88%ED%8F%AC%EA%B5%AC&prefix=%EB%A7%9B%EC%A7%91&size=10');
...
test('게시글 표시 개수를 바꾸면 URL과 API 요청에 반영한다', async ({ page }) => {
  const requests = [];
  page.on('request', (request) => {
    if (request.url().includes('/api/posts')) requests.push(request.url());
  });

  await page.goto('/posts?district=%EB%A7%88%ED%8F%AC%EA%B5%AC&page=3');
  await page.getByLabel('페이지당 표시 개수').selectOption('20');

  await expect(page).toHaveURL('/posts?district=%EB%A7%88%ED%8F%AC%EA%B5%AC&size=20');
  await expect.poll(() => requests.some((url) => new URL(url).searchParams.get('size') === '20')).toBe(true);
});
