...
const apiMessage = error => error.code === 'PASSWORD_MISMATCH' ? '비밀번호가 일치하지 않습니다.' : (error.message || '요청을 처리할 수 없습니다.');
...
export function mountCourseDetailPage({ outlet, params, signal, navigate }) {
  ...
  function renderEdit() {
    ...
    form.addEventListener('submit', async event => {
      ...
      catch (error) { if (error.name !== 'AbortError') { errorRoot.textContent = apiMessage(error); form.elements.password.value = ''; } }
  ...
  function confirmDelete(trigger) {
    const wrap = document.createElement('div'); const label = document.createElement('label'); label.textContent = '삭제 비밀번호'; const input = document.createElement('input'); input.type = 'password'; input.name = 'delete-password'; input.autocomplete = 'current-password'; const error = document.createElement('p'); error.className = 'course-error'; label.append(input); wrap.append(label, error);
    openModal({ title: '이 코스를 삭제할까요?', content: wrap, trigger, confirmLabel: '삭제', onConfirm: async () => { try { await deleteCourse(params.publicId, { password: input.value }, { signal }); input.value = ''; navigate('/courses'); return true; } catch (requestError) { error.textContent = apiMessage(requestError); input.value = ''; return false; } } });
  }
  ...
  getCourse(params.publicId, { signal }).then(data => { course = data; renderRead(); }).catch(error => {
    ...
    else renderAsyncState(root, { kind: 'error', message: apiMessage(error), onRetry: () => location.reload() });
  });
  return () => {};
}
